...
def generate_api_test(test_name):
    ...
    for i in range(1, max_range):
        ...
        x.append(i)
        function_time.append(counter.get_function_time())
        nuscr_time.append(counter.get_nuscr_time())
        type_time.append(counter.get_type_time())
        efsm_time.append(counter.get_efsm_time())
        class_time.append(counter.get_class_time())

    plot_graph(x, function_time, type_time, nuscr_time, efsm_time, class_time, test_name)


def plot_graph(x, function_time, type_time, nuscr_time, efsm_time, class_time, test_name):
    ...
    plt.xlabel('Number of Send/Receives')
    plt.ylabel('Time taken (s)')
    plt.title('Multiple Send/Receive with Two Participants')
    plt.legend()
